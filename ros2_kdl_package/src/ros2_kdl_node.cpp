...
class Iiwa_pub_sub : public rclcpp::Node
{
    public:
        ...
        node_handle_(std::shared_ptr<Iiwa_pub_sub>(this))
        {
            ...
            trajectory_point p = planner_.compute_trajectory_circular_polinomial(t);
    ...
    private:
        ...
        void cmd_publisher(){
            ...
            if (t_ < total_time){
                ...
                trajectory_point p = planner_.compute_trajectory_circular_polinomial(t_); 
                ...
                if(cmd_interface_ == "position"){
                    ...
                    KDL::Frame desPos;
                    desPos.p = KDL::Vector(p.pos.x(), p.pos.y(), p.pos.z());
                    desPos.M = KDL::Rotation::Identity();
                    // Desired velocity
                    KDL::Twist desVel;
                    desVel.vel = KDL::Vector(p.vel.x(), p.vel.y(), p.vel.z());
                    desVel.rot = KDL::Vector(0.0, 0.0, 0.0);
                    // Desired acceleration
                    KDL::Twist desAcc;
                    desAcc.vel = KDL::Vector(p.acc.x(), p.acc.y(), p.acc.z());
